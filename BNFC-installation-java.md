...
(adding to the `.bash_profile` is important so that all future terminals/shells know about `CLASSPATH` ... also, if you change the  `.bash_profile` after you opened a terminal, this terminal will not know that  `.bash_profile` changed ... you can either do a `source ~/.bash_profile` or simply close the terminal and open a new one)
